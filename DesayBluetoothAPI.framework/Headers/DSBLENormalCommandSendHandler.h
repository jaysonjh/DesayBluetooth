...
/**
 @brief Sending CONNECT status  and do some job in the callback block.
 
 @param open CONNECT status, YES:send "at+connect=1". NO:send "at+connect=0".
 @param callback dataString:[Type: NSString;Error status: "ERR";Normal status: "0";] Err:[If there is no error, here is nil.]
 
 */
- (void)sendCONNECTOpen:(BOOL)open callback:(nonnull DSBLECentralAPIDataCallback)callback;
@end
