...
typedef void (^DSBLECentralManagerRestorePeripheralsCallback)(NSArray<CBPeripheral *>*_Nullable peripheralArray);
...
/**
 *  This block is use for restore in CBCentralManager. when the function link @'- (void)centralManager:(CBCentralManager *)central willRestoreState:(NSDictionary<NSString *, id> *)dict' in CBCentralManager class be called, this function will call.
 */
@property(nonatomic,copy)DSBLECentralManagerRestorePeripheralsCallback _Nullable centralManagerRestorePeripheralsCallback;
...
/**
 *  for restore function. created on 2017.9.11
 */
+ (DSBLECentralManager * _Nonnull)shareManagerWithRetoreKey:(NSString *_Nonnull)key;

/**
 *  for restore function. created on 2017.9.11
 */
+ (DSBLECentralManager * _Nonnull)shareManagerWithRetoreKey;
