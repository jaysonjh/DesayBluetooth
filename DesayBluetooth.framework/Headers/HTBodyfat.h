...
typedef NS_ENUM(NSInteger, HTBodyfatErrorType){
    ErrorTypeNone,         //!< 无错误(可读取所有参数)
    ErrorTypeImpedance,    //!< 阻抗有误,阻抗有误时, 不计算除BMI/idealWeightKg以外参数(写0)
    ErrorTypeAge,          //!< 年龄参数有误，需在 6   ~ 99岁(不计算除BMI/idealWeightKg以外参数)
    ErrorTypeWeight,       //!< 体重参数有误，需在 10  ~ 200kg(有误不计算所有参数)
    ErrorTypeHeight        //!< 身高参数有误，需在 90 ~ 220cm(不计算所有参数)
...
@property (nonatomic,assign) NSInteger            htBodyAge;      //!< 身体年龄,还有优化中暂不使用
...
@property (nonatomic,copy) NSDictionary*          htBodyfatRatingList;    //!< 脂肪率健康标准字典"瘦－标准－"“标准－－标准＋”“标准＋－偏胖”“偏胖－肥胖”
